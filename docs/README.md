...
- Cập nhật `05_traceability-matrix.md` status từ ⬜ → ✅ khi test pass.
- Cập nhật status PRD/design từ `Draft` → `Approved`.
...
### 9.6. Status discipline

| Status | Ý nghĩa |
| - | - |
| `Draft` | Đang viết, chưa request review |
| `In Review` | Đã sẵn sàng review, có thể chạy gate |
| `Approved` | Reviewer ký rồi, không sửa nữa (sửa = changelog hoặc feature mới) |
